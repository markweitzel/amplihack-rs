...
fn run_blarify_indexing(
    ...
    if background {
        let child = cmd
            ...
            .stderr(Stdio::null())
            .spawn()?;
        amplihack_memory::memory::record_background_index_pid(project_root, child.id())?;
    } else {
...
fn resolve_blarify_index_action(
    status: &amplihack_memory::memory::IndexStatus,
    json_path: &Path,
) -> BlarifyIndexAction {
...
mod tests {
    ...
    fn setup_blarify_indexing_reuses_existing_background_job() {
        ...
        amplihack_memory::memory::record_background_index_pid(dir.path(), std::process::id())
            .unwrap();
