...
pub(super) fn check_version(dirs: &ProjectDirs) -> Option<String> {
    ...
    // Mirrors the workspace version-resolution convention: prefer the
    // build-time `AMPLIHACK_RELEASE_VERSION` override, fall back to Cargo.toml.
...
mod tests {
    ...
    use amplihack_memory::memory::resolve_code_graph_db_path_for_project;
    use std::fs;
