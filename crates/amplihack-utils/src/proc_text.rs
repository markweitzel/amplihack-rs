...
/// in `resolve_subprocess_safe` (amplihack-cli launch command) — both
...
mod tests {
    ...
    fn run_output_with_timeout_terminates_child_without_path() {
        ...
        // Give the *child* an empty PATH (the script and /bin/sleep use absolute
        // paths, so this exercises the "no PATH" scenario) without mutating the
        // parent process's global PATH — that global mutation would race with the
        // many sibling unit tests in this binary that spawn commands by name.
