...
use crate::proc_text::{run_output_with_timeout, strip_ansi, truncate_chars_with_notice};
...
mod tests {
    ...
    fn find_falls_back_to_npm_global_when_not_on_path() {
        ...
        let _guard = crate::test_env::home_env_lock()
            .lock()
        ...
        // Point HOME at the temp dir so install_fallback_dirs() resolves the
        // fake ~/.npm-global/bin. We do NOT strip PATH: `needle-tool-xyz` is a
        // unique made-up name that cannot exist on the real PATH, so PATH lookup
        // is guaranteed to miss and fall through to the install dirs. Clobbering
        // the global PATH here would race with sibling unit tests in this binary
        // that spawn commands (git/echo/sh) by name.
